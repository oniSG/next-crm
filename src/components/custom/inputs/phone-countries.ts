...
export function parsePhoneNumber(value: string, fallbackCountry: CountryCode) {
    const normalizedValue = value.trim()
    const matchedCountry = PHONE_COUNTRIES.reduce<
        (typeof PHONE_COUNTRIES)[number] | undefined
    >((match, country) => {
        if (!normalizedValue.startsWith(country.dialCode)) return match
        if (!match || country.dialCode.length > match.dialCode.length) return country
        return match
    }, undefined)

    if (!matchedCountry) {
        const fallback = PHONE_COUNTRIES.find(
    ...
    return {
        ...
        number: normalizedValue.slice(matchedCountry.dialCode.length).trim(),
    }
}
